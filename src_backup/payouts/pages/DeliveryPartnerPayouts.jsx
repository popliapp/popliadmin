...
const DeliveryPartnerPayouts = () => {
  ...
  const { payouts, loading, error, getDeliveryPartnerPayouts, updatePayoutStatus } = usePayout();
  const [statusFilter, setStatusFilter] = useState("");

  useEffect(() => {
    getDeliveryPartnerPayouts(1, 10, statusFilter);
  }, [statusFilter]);
  ...
  const columns = [
    { id: "deliveryPartner", label: "Delivery Partner Name", minWidth: 170, render: (payout) => payout.deliveryPartner?.user?.name || 'N/A' },
    ...
    { id: "date", label: "Date", minWidth: 170, render: (payout) => new Date(payout.createdAt).toLocaleDateString() },
    {
      ...
      render: (payout) => (
        <Box>
          <Button onClick={() => navigate(`/payouts/delivery-partners/${payout.deliveryPartner?._id}`)}>View History</Button>
